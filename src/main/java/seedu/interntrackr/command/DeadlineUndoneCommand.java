...
/**
 * Marks a specific deadline linked to an application as undone.
 */
public class DeadlineUndoneCommand extends Command {
    private static final Logger logger = Logger.getLogger(DeadlineUndoneCommand.class.getName());

    private final int applicationIndex;
    private final int deadlineIndex;

    /**
     * Constructs a DeadlineUndoneCommand for the specified application and deadline index.
     *
     ...
     */
    public DeadlineUndoneCommand(int applicationIndex, int deadlineIndex) {
    ...
    public void execute(ApplicationList applications, Ui ui, Storage storage) throws InternTrackrException {
        ...
        logger.info("Executing DeadlineUndoneCommand for application index "
        ...
        if (!deadline.getIsDone()) {
            logger.warning("Deadline already marked as undone for application index "
                    + applicationIndex + " and deadline index " + deadlineIndex);
            throw new InternTrackrException("This deadline has already been marked as undone.");
        }

        deadline.setNotDone();
        assert !deadline.getIsDone() : "Deadline should be marked as undone";

        ui.showMessage("Okay! Marked this deadline as undone:");
        ui.showMessage(deadlineIndex + ". " + deadline.toString());

        storage.save(applications.getApplications());
        logger.info("Deadline marked as undone and saved successfully for application index "
                + applicationIndex + " and deadline index " + deadlineIndex);
    }
}
